...
/*		pthread_mutex_lock (&m_io);
	...
	if ( nRead>= 3) {
		...
		if (mau_buf[0] == 0x7e) {
			...
			if (len>0) {
				//pkg = (uint8 *) malloc(sizeof(uint8)*(len+1)); // there is also
				//pkg = (uint8 *) malloc(sizeof(uint8)*100); // there is also  
				//nRead=readport(fd,pkg,len+1);
				//printf("step1\n");
...
//		pkg[4] = (uint8) ((rand() % 255) + 1);   // between 1 and 255
		
		if (++Frame_id == 0)
			Frame_id++;
		pkg[4] = (uint8)Frame_id;
	}
	else
...
// READ GENERICA
void read_pkg(uint8 *buf, int len, void **load, int size_load) {
