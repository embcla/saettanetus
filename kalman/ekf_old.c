...
void init_ekf(){
	int i;
	float qN[] = {0.1, 0.1};			// Input NoiSe Covariance
	float vN[] = {0.1, 0.1, 0.01};		// System Noise Covariance
	float pN[] = {1, 1, 1};

	// Sensor Noise Covariance
	vectR = (float *) malloc(sizeof(float)*NUM_SENS);
	for (i=0;i<NUM_SENS;i++)
		vectR[i]=0.01;			// [1cm] (good for a laser)
...
//	// Sensor odo base
//	for (i=0; i<NUM_SENS; i++) {
//		sa[i][SX]=sd[i][0]*cos(sd[i][1]);
//		sa[i][SY]=sd[i][0]*sin(sd[i][1]);
//		sa[i][STH]=sd[i][2];		
//	}
	
	// SAETTA CONFIGURATION
	sa[0][SX]=-7.5;
	sa[0][SY]=9.0;
	sa[0][STH]=M_PI/2;		
