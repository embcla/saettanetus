...
#ifdef XBEE
    xbee_fd_r = open_port_read(XBEE_DEVICE, 0);
    xbee_fd_w = open_port_write(XBEE_DEVICE,0);
...
#ifdef	XBEE
    close_port(xbee_fd_w);
    close_port(xbee_fd_r);
...
//            Send_Data_16(xbee_fd, 0xff, pkg_trajectory, (void *)lala, sizeof(struct payload_pkg_trajectory), NO_ACK);
        Send_Data_16(xbee_fd_w, 0xff, pkg_state, (void *) &tosend, sizeof(struct payload_pkg_state), NO_ACK);
       	xbee_packet_sent++;
